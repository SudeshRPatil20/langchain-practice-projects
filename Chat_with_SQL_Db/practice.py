...
if not db_uri:
    print("Plese give Database Uri")
if not api_key:
    print("Please Enter Groq Api Key ")

#LLM model
llm = ChatGroq(api_key=api_key, model_name="llama3-8b", streaming=True)
...
if db_uri==MYSQL:
    db=configure_db( db_uri=db_uri,  mysql_host=mysql_host,  mysql_user=mysql_user, mysql_password=mysql_password, mysql_db=mysql_db)
else:
